"""
Feature Fusion Data Loader
--------------------------
This module provides a PyTorch Dataset designed to consume pre-computed DSP feature tensors (.npy)
generated by the offline extraction pipeline. 

Key behaviours:
1. Dynamic Merging: It can read multiple CSV manifests (e.g., one for Mel, one for CQT) and 
   perform an inner join to perfectly align disparate feature arrays for a single audio file.
2. Tensor Stacking: It loads variable-length feature matrices and stacks them into 
   multi-channel 3D tensors (Channels, Frequencies, Time) ready for CNN ingestion.
3. Multi-Hot Encoding: It parses string-based labels ("guzheng|erhu") into 
   binary multi-hot tensors for classification.
"""
...
class FeatureFusionDataset(Dataset):
    def __init__(
        ...
        cqt_manifest_path: Optional[Union[str, Path]] = None,
        feature_manifest_paths: Optional[Dict[str, Union[str, Path]]] = None,
        project_root: str = ".",
        transform=None,
        ...
        manifest_paths: Dict[str, Union[str, Path]] = dict(feature_manifest_paths or {})
        ...
        # 1. Load and safely merge DataFrames for the requested feature set.
        ...
        for feature_name in self.feature_names[1:]:
            ...
            # validate="one_to_one" ensures we don't accidentally multiply rows via duplicate keys
            df = pd.merge(df, df_feature, on=["wavpath", "label"], how="inner", validate="one_to_one")

        # 2. Canonicalise labels
        labels_series = df.get("labels", df.get("label"))
        if labels_series is None:
            raise ValueError("Merged manifest must contain a 'label' or 'labels' column")
    ...
    @staticmethod
    def _load_feature_dataframe(feature_name: str, manifest_path: Union[str, Path]) -> pd.DataFrame:
        ...
        # Drop duplicates to ensure safe merging later
        df = df.drop_duplicates(subset=["wavpath", "label"]).copy()
    ...
    def _load_npy(self, path_str: str) -> torch.Tensor:
        ...
        if not npy_path.exists():
            raise FileNotFoundError(f"Feature tensor not found: {npy_path}")
...
class SpecAugment:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        # x shape: (Channels, Frequencies, Time)
